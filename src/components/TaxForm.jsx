...
const TaxForm = () => {
  ...
  const handleApplyToItems = (values) => {
    console.log({
  ...
  const extractCategories = () => {
    const uniqueCategories = Array.from(
      new Set(itemsApiResponse.map((item) => item.category?.name || 'Uncategorized'))
    );
  ...
  return (
    <Formik initialValues={initialValues} onSubmit={handleApplyToItems}>
      {({ values, setFieldValue }) => (
        <Form>
          ...
          <label>
            Name
            <Field type="text" name="name" placeholder="four" />
          </label>

        
          <label>
            Rate
            <Field type="text" name="rate" placeholder="4%" />
          </label>
...
<div>
  <label>
    ...
    <div className="radio-container">
      ...
      <div className="radio-label">Apply to specific items</div>
    </div>
  </label>
  <label>
    ...
    <div className="radio-container">
      <div className="radio-circle"></div>
      <div className="radio-label">Apply to all items in collection</div>
    </div>
  </label>
</div>
  ...
  {({ push, remove }) => (
    <div>
      {itemsApiResponse.map((item) => (
        <div key={item.id}>
          {item.category && !values.categories[item.category.name] && (
            <div
              style={{
              ...
              <label>
                <Field
                  ...
                  onClick={() => {
                    ...
                    newCategories[item.category.name] = !newCategories[item.category.name];
          ...
          {(values.categories[item.category?.name] || !item.category) && (
            <div>
              <label>
                <Field
                  type="checkbox"
                  ...
                  onChange={(e) => {
                    ...
                    setFieldValue('applicableItems', updatedItems);
                  }}
                />
                {item.name}
              </label>
          ...
          <button type="submit" className="apply-tax-button">
            Apply Tax to {values.applicableItems.length} items
          </button>
        </Form>
      )}
    </Formik>
